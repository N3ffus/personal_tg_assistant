...
факта. Повторы отбрасываются, а слои чередуются, чтобы лимит не вытеснил нужный.
Собственного поиска поверх Neo4j нет.

Составное сообщение сохраняется несколькими фактами: «меня зовут X, родился
DD.MM.YYYY в городе Y» — это три отдельных `remember_knowledge`. Короткий факт
извлекается в граф надёжно, а по эпизодам поиск идёт только пословно, без учёта
морфологии, поэтому деталь, потерянная при извлечении составной фразы, может
стать ненаходимой.
