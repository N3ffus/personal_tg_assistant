...
class Scenario:
    ...
    # Exact number of recommended films the reply must list.
    films_shown: int | None = None
...
# The titles a model reaches for first, so a batch collides with the catalogue.
WATCHED_FILMS = (
    "Интерстеллар",
    "Побег из Шоушенка",
    "Зелёная миля",
...
SCENARIOS = (
    ...
    *(
    ...
    Scenario(
        ...
        "Сколько мне лет",
        ({"type": "chat", "text": "Тебе 24 года."},),
        (),
        "Посчитать возраст арифметикой: 17.11.2001 и текущая дата 05.09.2026 дают 24 года. "
        "Не называть другое число и не переспрашивать дату рождения. Ответить одной короткой "
        "фразой: без даты рождения, сегодняшней даты и показанного расчёта.",
        ("24",),
        knowledge_facts=(
            "Пользователь называется Громов Лев Аркадьевич",
            "Лесноград — город рождения Громова Льва Аркадьевича (род. 17.11.2001).",
        ),
    ),
    Scenario(
        "days_until_the_birthday_are_answered_briefly",
        "через сколько дней моя днюха",
        ({"type": "chat", "text": "Через 73 дня."},),
        (),
        "Посчитать дни от 05.09.2026 до ближайшего 17.11.2026 — 73. Ответить одной короткой "
        "фразой: без даты рождения, сегодняшней даты, даты следующего дня рождения и расчёта.",
        ("73",),
        knowledge_facts=(
            "Пользователь называется Громов Лев Аркадьевич",
            "Лесноград — город рождения Громова Льва Аркадьевича (род. 17.11.2001).",
    ...
    Scenario(
        ...
        reply_excludes=tuple(f"«{title}»" for title in WATCHED_FILMS),
        knowledge_facts=WATCHED_FILMS,
    ),
    *(
        Scenario(
            f"recommendations_fill_the_requested_count_{count}",
            prompt,
            (
                {
                    "type": "recommend_films",
                    "limit": count,
                    "candidates": [
                        {"title": f"Пример {index}", "reason": "Пример"}
                        for index in range(1, count + 1)
                    ],
                },
            ),
            (),
            f"Пользователь явно назвал число: показать ровно {count} непросмотренных "
            "фильмов действием recommend_films, а не три по умолчанию и не chat. "
            "Ни один фильм из каталога просмотренного (memory_lookups, "
            "knowledge.watched_film_titles) не должен попасть в ответ. Кандидаты "
            "внутри действия — заявка, которую приложение само сверяет с "
            "каталогом, поэтому суди по тексту ответа.",
            ("В твоём списке просмотренных нет",),
            reply_excludes=tuple(f"«{title}»" for title in WATCHED_FILMS),
            films_shown=count,
            knowledge_facts=WATCHED_FILMS,
        ...
        for count, prompt in [
            # Production answered this with two films (2026-09-15).
            (10, "Посоветуй топ 10 фильмов для меня"),
            (5, "Посоветуй пять фильмов, которые я ещё не видел"),
        ]
    ),
)
