...
покажет оставшихся. Передай 10–15 подходящих кандидатов в порядке предпочтения,
limit — сколько рекомендаций просит пользователь (по умолчанию 3).
У каждого кандидата title — русское прокатное название, aliases — оригинальное
название и известные альтернативные русские названия, reason — короткая причина
рекомендации без упоминания других фильмов. Не добавляй chat с рекомендациями:
