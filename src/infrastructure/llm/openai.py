...
ANSWER_TURN_INSTRUCTION = (
    ...
    "непросмотренных фильмов используй recommend_films вместо chat. Если нужного "
...
- semantic (по умолчанию) — поиск по смыслу. Жанры, конкретные названия,
  желания и любые другие факты о пользователе ищи только так.
...
покажет оставшихся.
limit — сколько рекомендаций показать пользователю (по умолчанию 3). Это НЕ число
кандидатов: кандидатов всегда 12–15, даже при limit=3. Каталог просмотренного
велик, и большинство кандидатов из него вычеркнут; если ты пришлёшь три, показать
будет нечего.
